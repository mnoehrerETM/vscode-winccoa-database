...
export class McpClient {
    ...
    private async callMcpTool(
        ...
        if (!this.config) {
            return {
                success: false,
                error: 'MCP client not configured. Is the MCP HTTP server running?',
            };
        }
        ...
        try {
            ...
            if (dataMatch) {
                ...
                if (result.error) {
                    const rpcError =
                        result.error.message || result.error.data || JSON.stringify(result.error);
                ...
                if (content) {
                    ...
                    try {
                        ...
                        log.error(
                            `[MCP] Failed to parse tool response: ${content.substring(0, 200)}`,
                        );
                    ...
                    return {
                        success: false,
                        error: parsed.message || parsed.error || `${toolName} returned failure`,
                        data: parsed,
                    };
                }
            }
...
export async function promptMcpSetup(): Promise<void> {
    ...
    for (const id of MCP_EXTENSION_IDS) {
        ...
        if (ext) {
            log.info(`[MCP Setup] Found extension: ${id} (active=${ext.isActive})`);
            mcpExt = ext;
            break;
        }
        log.info(`[MCP Setup] Extension ${id} not installed`);
    }
    ...
    if (mcpExt) {
        ...
        if (choice === install) {
            await vscode.commands.executeCommand(
                ...
                `@id:${MCP_EXTENSION_IDS[MCP_EXTENSION_IDS.length - 1]}`,
            );
        }
    }
}
